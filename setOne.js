...
let parReturn = (par1, par2) =>
  `The first paramater is ${par1}. The second parameter is ${par2}`;

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

console.log(parReturn("Discord", "Zoom"));
...
const fullGreeting = function (fullName) {
...
greeting("Brenda", "Escobedo", fullGreeting);
...
function pricesPlusTax(arr, cb) {
  for (let i = 0; i < arr.length; i++) {
    totalCost.push(arr[i] + arr[i] * 0.2);
  }
  cb(totalCost);
}

let grandTotal = function (totalCost) {
...
pricesPlusTax(prices, grandTotal);
...
let multiplyingFactory = function (num1) {
